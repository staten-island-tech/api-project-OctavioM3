...
async function getData(EquipmentURL, MonsterURL, MaterialURL){
  try {
    ...
    const secresponse = await fetch(MonsterURL);
    const thirdresponse = await fetch(MaterialURL);
    if (response.status != 200, secresponse.status != 200, thirdresponse.status != 200) {
      throw new error(response.statusText, secresponse.statusText, thirdresponse.statusText);
    }
...
function inputMaterials() {
  materialdata.data.forEach((materialdata) => DOMSelectors.ItemBox.insertAdjacentHTML(
    "beforeend",
